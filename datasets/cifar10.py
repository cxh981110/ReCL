...
class CIFAR10_LT(object):
    ...
    def __init__(self, distributed, root='./data/cifar10', imb_type='exp',
                    imb_factor=0.01, batch_size=256, num_works=40):
        ...
        train_dataset = IMBALANCECIFAR10(root=root, imb_type=imb_type, imb_factor=imb_factor, rand_number=0, train=True, download=True, transform = ThreeCropTransform(train_transform))
        ...
        support, query, support_y, query_y = train_test_split(train_images, train_labels, test_size=0.8, random_state=0, stratify=train_labels)
        support_t = np.transpose(support,(0,3,1,2))
        support_y_tensor = torch.tensor(support_y)
        support_tensor = torch.tensor(support_t).float()

        # # build support set
        self.support_dataset = TensorDataset(support_tensor, support_y_tensor)
        self.support_dataset.transform = Lambda(lambda x: train_transform(x))
        self.support_loader = DataLoader(self.support_dataset, batch_size=128, shuffle=True)
