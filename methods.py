...
def mixup_data(x, x_1, y, alpha=1.0, use_cuda=True):
    ...
    return mixed_x, mixed_x, y_a, y_b, lam


def mixup_criterion(criterion, pred, y_a, y_b, lam):
    return lam * criterion(pred, y_a) + (1 - lam) * criterion(pred, y_b)

def mixup_supervised(criterion_cs, config, feat, center, y_a, y_b, lam):

    return lam * criterion_cs(config, feat, center, y_a) + (1 - lam) * criterion_cs(config, feat, center, y_b)
